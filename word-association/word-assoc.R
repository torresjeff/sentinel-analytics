...
palabras_corrupcion_assoc <- read.csv(file="palabras-corrupcion-assoc.txt", header=FALSE, sep=",", encoding="UTF-8")
...
palabras_message <- generate_regex("whole_sentence", palabras_corrupcion_assoc[,1])
#palabras_message
palabras <- c()
#palabras <- c(rbind(palabras_message))
palabras <- paste(unlist(palabras_message), collapse=',')
#palabras
...
for (i in 1:length(palabras_corrupcion_assoc[,4])) {
  print(i)
  print(palabras_corrupcion_assoc[i,4])
}
